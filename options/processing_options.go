...
var defaultURLWatermarkOptions = UrlWatermarkOptions{
	WatermarkOptions: WatermarkOptions{
		Opacity:  1,
		Position: GravityOptions{Type: GravityCenter},
	},
}
...
func applyURLWatermarkOption(po *ProcessingOptions, args []string) error {
	if len(args) > 8 {
		return newOptionArgumentError("Invalid url watermark arguments: %v", args)
	}
	...
	if decoded, err := url.QueryUnescape(args[0]); err == nil {
		...
		return newOptionArgumentError("Invalid url watermark url: %s", args[0])
	}
	...
	if len(args) > 1 && len(args[1]) > 0 {
		if o, err := strconv.ParseFloat(args[1], 64); err == nil && o >= 0 && o <= 1 {
			...
			return newOptionArgumentError("Invalid url watermark opacity: %s", args[1])
		}
	}
	...
	if len(args) > 5 && len(args[5]) > 0 {
		if s, err := strconv.ParseFloat(args[5], 64); err == nil && s >= 0 {
			...
			return newOptionArgumentError("Invalid url watermark scale: %s", args[4])
		}
	}
